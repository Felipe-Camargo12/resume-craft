  ...
  return (
    <button
      ...
      <div className="absolute w-full left-0 bottom-0 p-3 text-left bg-gradient-to-t from-background/80">
        ...
        <span className="block text-xs text-muted-foreground">{descripiton}</span>
      </div>
    </button>
  );
};
...
export const ResumeCard = () => {
  return (
    <Link href="/dashboard/resume/example" className="block w-full">
        <ResumeCardButton
            title="Meu currículo"
            descripiton="Última atualização há 22 minutos"
        />
    </Link>
  );
};
